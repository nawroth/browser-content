= Get started with Neo4j Browser
...
[horizontal]
Intro:: a guided tour of Neo4j browser [runnable]`:play intro`
...
Custom Guides:: starting with Neo4j 2.3 you can use `:play <url>` to play a custom guide, for example
  `:play http://guides.neo4j.com/intro/create.html` from the fundamentals training.
...
* since Neo4j 2.3 there is a config drawer on the left (with the cog icon:cog[]), no need for the `:config` command
anymore
